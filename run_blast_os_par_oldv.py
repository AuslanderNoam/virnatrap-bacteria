...
def get_table_reference(infile, outfie, filen):
    ...
    smp_name = [infile.split('/')[-1].split('.')[0].replace('_gdc_realn_rehead', '').replace('_contigs','') for i in uni_vir]
...
def get_table_herv(infile,outfie,filen,prc_thr = 50):
    '''
    :param infile: input file (contigs-fasta), at dbgap-mutporg/
    :param outfie: an output file from input file
    :param filen: generated files name (dirname_ref+'/'+file.split('/')[-1].split('.')[0])
    :return: csv data with LTR and ERV/HERV
    '''
    findi = lambda x, v: [i for i, j in enumerate(x) if j == v]

    col_names = ["id", "names", "score", "coverage", "b", "c", "d", "e", "f", "g", "h", "i"]
    out11 = pd.read_csv(outfie, sep='\t', header=None, names=col_names)
    ...
    du_name=out11['names']
    name = sorted(list(set(du_name)))
    uni_ltr = [i.split('_')[2] for i in name]

    erv = []
    for i in sorted(list(set(name))):

        m = i.split('_')[1:]
        a = [a for a in m if 'ERV' in a]
        b = [b for b in m if 'HERV' in b]
    ...
    for i in range(len(name)):
        indexes = [k for k in findi(du_name, name[i])]
        avg = np.mean([(score[k]) for k in indexes])
        lens = [all_len[k] for k in indexes]
        average.append(avg)
        cover_av.append(np.mean([(coverage[k]) for k in indexes]))
        seq_len.append(lens)
        con = list(set([all_con[k] for k in indexes]))
    ...
    smp_name = [infile.split('/')[-1].split('.')[0].replace('_gdc_realn_rehead', '').replace('_contigs','') for i in name]

    case_id, sample_tp, project_id = get_project_id(smp_name)
    df_0 = pd.DataFrame({'case_id': case_id, 'sample': smp_name, 'ltr': uni_ltr, 'erv': erv, 'full_name': name,
                       'avg_similarity': average,'avg_coverage': cover_av, 'lengths': seq_len, 'contigs':contig, 'project_id': project_id,
                       'sample_type': sample_tp})

    df_1 = df_0.set_index('case_id')
    df = df_1.dropna(subset=['erv'])

    idx = np.unique([i for i in range(len(df.index)) if
                     ((np.max(df['lengths'][i]) > 200 and list(df['avg_coverage'])[i] > 95) and (list(df['avg_similarity'])[i] > 95)) and list(df['erv'])[i] not in contaminators])
...
def get_table_phage(infile, outfie, filen, parse_dict= False ,prc_thr = 40):
    '''
    :param infile: input file (contigs-fasta), at dbgap-mutporg/
    :param outfie: An output file from input file
    :param filen: generated files name (dirname_ref+'/'+file.split('/')[-1].split('.')[0])
   ...
   '''
    ...
    virs = list(out11['virus'])
    #vir = [i.split('|')[1] for i in virs]
    vir = [i.split('.')[0] for i in virs]
    ...
    if parse_dict:
        ...
        for gb_record in SeqIO.parse('sequence.gb', 'genbank'):
           ...
           tax_dict[gb_record.annotations['accessions'][0]] = tax_line
           if 'host' in gb_record.features[0].qualifiers:
                host_line = ''.join(gb_record.features[0].qualifiers['host'])
                host_dict[gb_record.annotations['accessions'][0]] = host_line
           else:
                host_dict[gb_record.annotations['accessions'][0]] = np.nan

        with open('gb_ref.json', 'w') as fp:
            json.dump(tax_dict, fp, indent=4)

        with open('host_ref.json', 'w') as f:
            json.dump(host_dict, f, indent=4)

    else:
        with open('gb_ref.json') as fp:
            tax_dict = json.load(fp)

        with open('host_ref.json') as f:
    ...
    host = []
    for i in range(len(uni_vir)):
        indexes = [k for k in findi(vir, uni_vir[i])]
        avg=np.mean([(score[k]) for k in indexes])
        ...
        contig.append(con)
        seq_len.append(get_value(dic_con, con))
        if uni_vir[i] not in tax_dict.keys():
            f_1.append(np.nan)
            f_2.append(np.nan)
        else:
            f_1.append(tax_dict.get(uni_vir[i]).split(';')[-1])
            f_2.append(tax_dict.get(uni_vir[i]).split(';')[-2])
        if uni_vir[i] not in host_dict.keys():
            host.append(np.nan)
        else:
            host.append(host_dict.get(uni_vir[i]))
    ...
    df_0=pd.DataFrame({'case_id':case_id, 'sample':smp_name,'virus':uni_vir,'host' : host, 'family_1': f_1,
                     'family_2' : f_2, 'avg_similarity':average, 'avg_coverage': cover_av,'lengths': seq_len,'contigs':contig,
                     'project_id':project_id,'sample_type':sample_tp})

    df = df_0.set_index('case_id')

    idx = np.unique([i for i in range(len(df.index)) if
                     ((np.max(df['lengths'][i]) > 100 and list(df['avg_coverage'])[i] > 90) and (list(df['avg_similarity'])[i] > 90)) and list(df['virus'])[i] not in contaminators])

    df = df.iloc[idx]

    # nameadd+filen.replace('_contigs','')
    df.to_csv(filen + '.csv')
    return df

def get_table_bact(infile,outfie,filen,prc_thr = 50):
    '''
    :param infile: input file (contigs-fasta), at dbgap-mutporg/
    :param outfie: an output file from input file
    :param filen: generated files name (dirname_ref+'/'+file.split('/')[-1].split('.')[0])
    :return: csv data with bacteria???
    '''
    findi = lambda x, v: [i for i, j in enumerate(x) if j == v]
    ...
    for i in range(len(name)):
        ...
        cover_av.append(np.mean([(coverage[k]) for k in indexes]))
        seq_len.append(lens)
        if name[i] in tax_dict:
            tax.append(tax_dict[name[i]])
        else:
            tax.append("?")
        con = list(set([all_con[k] for k in indexes]))
        contig.append(con)
    ...
    case_id, sample_tp, project_id = get_project_id(smp_name)
    df_0 = pd.DataFrame({'case_id': case_id, 'sample': smp_name, 'accession': name, "seq_name": tax,
                       'avg_similarity': average,'avg_coverage': cover_av, 'lengths': seq_len, 'contigs':contig, 'project_id': project_id,
                       'sample_type': sample_tp})
    ...
    idx = np.unique([i for i in range(len(df.index)) if
                     ((np.max(df['lengths'][i]) > 200 and list(df['avg_coverage'])[i] > 95) and (list(df['avg_similarity'])[i] > 95))])
...
def combine_csv(indir,outname,extension = 'csv',prc_thr = 50):
    '''
...
def get_blast_results(params):
    ...
    fn = dirname_herv + '/' + file.split('/')[-1].split('.')[0]
    if not exists(fn):
        subprocess.run('blastn -db $BLASTDB/herv.fa -query ' + file + '  -outfmt "6 qseqid sseqid pident qcovs length mismatch gapopen qstart qend sstart send evalue" -max_target_seqs 1 -evalue 0.01 | sort -u > '+output_dir+'tmpout_herv'+file.split('/')[-1].split('.')[0]+'.txt',shell=True)
        get_table_herv(file, output_dir+'tmpout_herv'+file.split('/')[-1].split('.')[0]+'.txt', dirname_herv + '/' + file.split('/')[-1].split('.')[0])
        subprocess.run('rm -rf '+ output_dir+'tmpout_herv'+file.split('/')[-1].split('.')[0]+'.txt',shell=True)

    fn = dirname_phage + '/' + file.split('/')[-1].split('.')[0]
    if not exists(fn):
        subprocess.run('blastn -db $BLASTDB/phage.fasta -query ' + file + '  -outfmt "6 qseqid sseqid pident qcovs length mismatch gapopen qstart qend sstart send evalue" -max_target_seqs 1 -evalue 0.01 | sort -u > ' + output_dir+  'tmpout_phage'+file.split('/')[-1].split('.')[0]+'.txt',shell=True)
        get_table_phage(file, output_dir + 'tmpout_phage'+file.split('/')[-1].split('.')[0]+'.txt', dirname_phage + '/' + file.split('/')[-1].split('.')[0])
        subprocess.run('rm -rf ' + output_dir + 'tmpout_phage'+file.split('/')[-1].split('.')[0]+'.txt',shell=True)

    fn = dirname_bact + '/' + file.split('/')[-1].split('.')[0]
    if not exists(fn):
        subprocess.run('blastn -db $BLASTDB/bacteriaref -query ' + file + '  -outfmt "6 qseqid sseqid pident qcovs length mismatch gapopen qstart qend sstart send evalue" -max_target_seqs 1 -evalue 0.01 | sort -u > ' + output_dir+  'tmpout_bact'+file.split('/')[-1].split('.')[0]+'.txt',shell=True)
        get_table_bact(file, output_dir + 'tmpout_bact'+file.split('/')[-1].split('.')[0]+'.txt', dirname_bact + '/' + file.split('/')[-1].split('.')[0])
        subprocess.run('rm -rf ' + output_dir + 'tmpout_bact'+file.split('/')[-1].split('.')[0]+'.txt',shell=True) 
...
if __name__ == "__main__":
    ...
    os.system('mkdir ' + output_dir + 'out_herv')
    os.system('mkdir ' + output_dir + 'out_phage')
    os.system('mkdir ' + output_dir + 'out_ref')
    os.system('mkdir ' + output_dir + 'out_bact')

    dirname_ref = output_dir + 'out_ref'
    dirname_novel = output_dir + 'out_novel'
    dirname_herv = output_dir + 'out_herv'
    dirname_phage = output_dir + 'out_phage'
    dirname_bact = output_dir + 'out_bact'
    

    files = glob.glob(args.input + '*.txt')
    
    try:
        files.remove("/wistar/dbgap-mutporg/download_tools/GTEx/ESOPHAGUS/Results/assembly_output2/GTEX-OIZF-0626-SM-2I5GT_contigs.txt") #This is the in progress ESO sample
    except ValueError:
        pass 
    print(len(files))
    if __name__ == "__main__":
    ...
    combine_csv(dirname_herv, dirname_herv + "/combined_csv_herv.csv")
    combine_csv(dirname_ref, dirname_ref + "/combined_csv_ref.csv")
    combine_csv(dirname_phage, dirname_phage + "/combined_csv_phage.csv")
    combine_csv(dirname_novel, dirname_novel + "/combined_csv_novel.csv")
    combine_csv(dirname_bact, dirname_bact + "/combined_csv_bact.csv")